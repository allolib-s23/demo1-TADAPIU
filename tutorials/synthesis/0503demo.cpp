...
public:
  ...
  void onProcess(AudioIOData &io) override
  {
    ...
    mOsc2.freq(2*getInternalParameterValue("frequency"));
...
// GUI manager for SineEnv voices
  ...
  /*void playSineEnv(float freq, float time, float duration, float amp = .1, float attack = 0.11, float decay = 0.01)
  {
...
/*
  ...
  void CanonD(int time){
    playNote(Fsharp5, measure * 0 + beat * 0 + time, whole);
    //playSineEnv(D2, measure * 0 + beat * 0 + time, whole);
    playNote(E5, measure * 1 + beat * 0 + time, whole);
